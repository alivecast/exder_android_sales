...
function getonMSG(json, callfn) {
	...
	console.log('getonMSG: ' + JSON.stringify(json));
...
// ファイル書込処理(書き込み成功ならokfn関数を実行、失敗ならerrfn関数を実行)
function setFileApi(fname, text, okfn, errfn) {
		...
		fileSystem.root.getFile(fname, {
			...
			fileEntry.createWriter(function(writer) {
				writer.onwrite = function(evt) {
					console.log("FileApi 書き込み成功");
				};
				writer.onerror = function(evt) {
					console.log('FileApi 書き込みに失敗' + evt.toString());
				};
				// 内容を書き込み
            	writer.write(text);
                callback = $FN(okfn);
...
// ファイル読み込み処理(読み込み成功ならokfn関数を実行、失敗ならerrfn関数を実行)
function getFileApi(fname,okfn,errfn) {
	var text = {};
	window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, read, noFileApi);
		...
		fileSystem.root.getFile(fname, {}, function(fileEntry) {
			fileEntry.file(function(file) {
				...
				reader.onloadend = function(evt) {
                    ...
                    console.log("FileApi 正常にファイルを読み込みました。");
                    callback = $FN(okfn);
					return callback(text);
				};
